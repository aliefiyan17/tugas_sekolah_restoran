...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <h2 class="text-4xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent mb-2">
            ✏️ Edit Meja {{ $meja->nomor_meja }}
        </h2>
        <p class="text-gray-600">Perbarui informasi meja</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <form action="{{ route('meja.update', $meja->idmeja) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="form-group">
                    ...
                    >
                        ...
                        <option value="2" {{ old('kapasitas', $meja->kapasitas) == 2 ? 'selected' : '' }}>2 Orang (Meja Kecil)</option>
                        <option value="4" {{ old('kapasitas', $meja->kapasitas) == 4 ? 'selected' : '' }}>4 Orang (Meja Sedang)</option>
                        <option value="6" {{ old('kapasitas', $meja->kapasitas) == 6 ? 'selected' : '' }}>6 Orang (Meja Besar)</option>
                        <option value="10" {{ old('kapasitas', $meja->kapasitas) >= 10 ? 'selected' : '' }}>10+ Orang (Meja XL)</option>
                    </select>
                    @error('kapasitas')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    >
                        ...
                        <option value="Indoor" {{ old('lokasi', $meja->lokasi) == 'Indoor' ? 'selected' : '' }}>
                            Indoor (Dalam Ruangan)
                        </option>
                        <option value="Outdoor" {{ old('lokasi', $meja->lokasi) == 'Outdoor' ? 'selected' : '' }}>
                            Outdoor (Luar Ruangan)
                        </option>
                        <option value="VIP" {{ old('lokasi', $meja->lokasi) == 'VIP' ? 'selected' : '' }}>
                            VIP (Ruang Khusus)
                        </option>
                    </select>
                    @error('lokasi')
                ...
                <div class="form-group">
                    ...
                    <select 
                        id="status" 
                        name="status" 
                        class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition @error('status') border-red-500 @enderror"
                        required
                    >
                        <option value="tersedia" {{ old('status', $meja->status) == 'tersedia' ? 'selected' : '' }}>
                            ✅ Tersedia
                        </option>
                        <option value="terisi" {{ old('status', $meja->status) == 'terisi' ? 'selected' : '' }}>
                            🔴 Terisi
                        </option>
            ...
            <div class="mt-6">
                ...
                >{{ old('keterangan', $meja->keterangan) }}</textarea>
                @error('keterangan')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-between items-center mt-8 pt-6 border-t-2">
                ...
                <button type="submit" class="px-8 py-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white rounded-xl font-semibold shadow-lg transition transform hover:scale-105">
                    <i class="fas  mr-2"></i>Update Meja
                </button>
            </div>
        </form>
    </div>
...
<script>
    function updatePreview() {
        ...
        let color = '#10b981'; // Green for tersedia
        if (status === 'terisi') color = '#ef4444'; // Red
        if (status === 'reserved') color = '#f59e0b'; // Yellow
        ...
        let statusEmoji = status === 'tersedia' ? '✅' : (status === 'terisi' ? '🔴' : '⏰');
        let statusText = status === 'tersedia' ? 'Tersedia' : (status === 'terisi' ? 'Terisi' : 'Reserved');
        ...
        previewDiv.innerHTML = `
            <div class="text-center">
                ...
                <div class="mt-4">
                    ...
                    <p class="text-lg font-semibold mt-2">${statusEmoji} ${statusText}</p>
                </div>
            </div>
        `;
    }
